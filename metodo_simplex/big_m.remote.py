...
def make_inequalities_equalities(constraints:list, f_o: dict, max:bool) -> None:
    """
    <summary>
    ...
    for i in constraints:
        ...
        if ( i['symbol'] in ('=', '>=', '=>') ): # (i['symbol'] == '=') and ((i['symbol'] == '>=') or (i['symbol'] == '=>'))
            ...
            f_o.update( {f'a{str(n)}': (-1, 'M')} ) # Agrego las variables artificiales a la función objetivo.
        # '<=' or '<'
        if ( i['symbol'] in ('<=', '<', '=<') ):
...
#     ]
    ...
    make_inequalities_equalities(constraints=constraints, f_o=f_o)
    # with open("result.txt", mode="a+") as file:
    #     file.truncate(0)
    #     file.close()
